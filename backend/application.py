...
# --- PATH FIX FOR SERVER ---
# Gets the full path to the directory where this .py file is running
BASE_DIR = os.path.dirname(os.path.abspath(__file__))
# Creates full, absolute paths to your model and data files
MODEL_PATH = os.path.join(BASE_DIR, 'LinearRegressionModel.pkl')
DATA_PATH = os.path.join(BASE_DIR, 'Cleaned_Car_data.csv')
# ---------------------------
...
try:
    ...
    print(f"Attempted to load data from: {DATA_PATH}")
    model = None
    car = pd.DataFrame(columns=['company', 'name', 'year', 'fuel_type']) # Fallback
...
# as the server (Gunicorn) will run the 'app' object directly.
